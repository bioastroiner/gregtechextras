		...
		if (parts != null) {
			...
			for (int i = 0; i < parts.length; i++) {
				if (parts[i] != null) {
					...
					if (data != null && (data.mPrefix == OP.plate || data.mPrefix == OP.plateGem || data.mPrefix == OP.plateDense)) {
						// Weight
						def[0] = def[0] + Values.INSTANCE.getValues(data.mMaterial.mMaterial).weight;
						if (data.mPrefix == OP.plateDense && data.mMaterial.mMaterial == MT.Iridium) {
							def[0] = def[0] - 20;
						}
						// physicalDef
